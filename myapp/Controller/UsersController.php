...
class UsersController extends AppController{
    ...
    public function signup(){
        if (!empty($this->request->data)) {
            ...
            if($this->User->validates()){
                ...
                $CakeEmail->subject('Votre inscription Pikshares');
                $CakeEmail->viewVars(
                ...
                $this->Session->setFlash('Merci vous êtes inscrit', 'flash', array('class' => 'success round'));
            }else{
                $this->Session->setFlash('Merci de corriger vos erreurs', 'flash', array('class' => 'warning round'));
            }
        }
    }
    ...
    public function forgot(){
        if (!empty($this->request->data)) {
            ...
            if(empty($user)){
                $this->Session->setFlash("Ce email n'est associé a aucun compte","flash", array('class' => 'warning round'));
            }else{
                ...
                $this->Session->setFlash("Un email vous a été envoyé avec les instructions pour regénérer votre mot de passe","flash", array('class' => 'success round'));
            }
        }
    }
    ...
    public function password($user_id, $token){
        ...
        if (empty($user)) { 
            $this->Session->setFlash('Ce lien ne semble pas bon', 'flash', array('class' => 'warning round'));
        ...
        if(!empty($this->request->data)){
            ...
            if($this->User->validates()){
                ...
                $this->Session->setFlash("Votre mot de passe a bien été modifié","flash", array('class' => 'success round'));
    ...
    public function activate($user_id, $token){
        ...
        if (empty($user)) {
            $this->Session->setFlash('Ce lien de validation ne semble pas bon', "flash", array('class' => 'warning round'));
            return $this->redirect('/');
        }
        $this->Session->setFlash('Votre compte a bien été validé', "flash", array('class' => 'success round'));
        $this->User->save(array(
            'id'     => $user['User']['id'],
            'active' => 1,
            'token'  => ''
